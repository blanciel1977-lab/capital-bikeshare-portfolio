...
## 14. 이상치 1차 원인 변수 분해 시각화

`good_templet.md`에 "남은 확인 과제"로 남겨뒀던 항목(jskim414가 런던에도 적용했던 이상치 root-cause 분해)을 사용자가 "1차 원인변수를 잘 나타내는 시각화" 요청으로 실제 수행했다.

- `19_anomaly_root_cause.py` 신규: 워싱턴과 동일한 z-score 방식(연속형 t2/hum/wind_speed/cnt 중 전체 727일 평균 대비 |z-score| 최대인 변수)으로 37건 이상치를 분해, 원인 빈도 막대그래프 + t2×hum 산점도(원인별 색 구분) 2패널 이미지 생성
- 결과: `cnt` 10건(27%), `wind_speed`/`hum`/`t2` 각 9건(24%)으로 **워싱턴(습도가 38%로 지배적)과 달리 네 변수가 고르게 분포** — 지배적 원인이 하나로 좁혀지지 않는다는 결론
- jskim414가 보고한 "실기온(14) > 습도(9)"와도 다른데, jskim414는 `cnt`를 원인 후보에서 제외했을 가능성이 있어 "원인 후보를 무엇으로 두느냐 자체가 결론을 바꾼다"는 추가 시사점을 `good_templet.md`에 남김
- README.md 이상치 절, REPORT.md 5절에 결과 반영
